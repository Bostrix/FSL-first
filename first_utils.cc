...
string title="first_utils University of Oxford (Brian Patenaude)";
...
Option<bool> useReconMNI(string("--useReconMNI"), false,
                         ...
                         false, no_argument);
Option<bool> saveAverageMesh(string("--saveAverageMesh"), false,
                         string("Save average mesh in MNI space (to be "
                                "used with --usebvars --vertexAnalysis)"),
                         false, no_argument);
...
Option<string> pathname(string("-a,--extra_image"), string(""),
                        string("Specifies extra path to image in .bvars file"),
                        false, requires_argument);
Option<string> flirtmatsname(string("-f,--flirtmats"), string(""),
                             string("Text file containing filenames of flirt matrices (filenames, not numbers)."),
                             false, requires_argument);
Option<string> meshname(string("-m,--mesh"), string(""),
                        string("Filename of input mesh"),
                        false, requires_argument);
Option<string> normname(string("-g,--factors"), string(""),
                        string("Filename of normalization factors."),
                        false, requires_argument);
Option<string> designname(string("-d,--design"), string(""),
                          string("Filename of fsl design matrix"),
                          false, requires_argument);

Option<string> refname(string("-r,--reference"), string(""),
...
void do_work_bvars(){
  ...
  if (!vertexAnalysis.value()){
    ...
    // save average mesh as vtk if requested
    if (saveAverageMesh.value()) {
      fslvtkIO * avmeshout = new fslvtkIO();
      Matrix vertices(avMesh._points      .size(), 3);
      Matrix indices( avMesh.loc_triangles.size(), 3);
      for (size_t i = 0; i < avMesh._points.size(); i++) {
        auto pt = avMesh._points[i]->get_coord();
        vertices(i + 1, 1) = pt.X;
        vertices(i + 1, 2) = pt.Y;
        vertices(i + 1, 3) = pt.Z;
      }
      for (size_t i = 0; i < avMesh.loc_triangles.size(); i++) {

        auto tri = avMesh.loc_triangles[i];
        indices(i + 1, 1) = tri->get_vertice(0)->get_no();
        indices(i + 1, 2) = tri->get_vertice(1)->get_no();
        indices(i + 1, 3) = tri->get_vertice(2)->get_no();
      }
      avmeshout->setPoints(  vertices);
      avmeshout->setPolygons(indices);

      avmeshout->save(outname.value() + "_average.vtk");
    }
