...
Option<bool> intref(string("--intref"), false,
                    string("use structure specified by modelname2 as intensity reference"),
                    false, no_argument);

Option<bool> multiImageInput(string("--multiImageInput"), false,
                             string("Process multiple input images. The value passed as --in must "
                                    "be a text file containing 'image flirtmat outname' triples. "
                                    "separated with spaces and/or newlines."),
                             false, no_argument);

Option<bool> binarySurfaceOutput(string("--binarySurfaceOutput"), false,
                                 string("Save surface data as raw binary in output VTK files"),
                                 false, no_argument);
